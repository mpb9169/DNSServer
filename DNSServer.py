...
def decrypt_with_aes(encrypted_data, password, salt):
    key = generate_aes_key(password, salt)
    f = Fernet(key)
    # If encrypted_data is a string (from DNS TXT record), encode it back to bytes
...
# Convert encrypted bytes to string for storage in TXT record (keeps it encrypted!)
...
dns_records = {
    ...
    # Simple A records for various domains
    'safebank.com.': {
    ...
    'nyu.edu.': {
        ...
        dns.rdatatype.TXT: (encrypted_string,),  # Encrypted secret data as TXT record
